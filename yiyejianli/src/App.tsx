...
import { saveResume, getResume, getResumesList, saveResumeWithId, deleteResume, renameResume, createNewResume, ResumeDocument } from './lib/firebaseService';
...
export default function App() {
  ...
  const refreshResumesList = async (uId: string, isDemo: boolean) => {
    try {
      const list = await getResumesList(uId, isDemo);
      setResumes(list);
      return list;
    } catch (e) {
      console.error(e);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        const list = await refreshResumesList(currentUser.uid, currentUser.isAnonymous);
        if (list.length > 0) {
        ...
        // Initialize App User state (simulate backend data)
        setAppUser({
          id: currentUser.uid,
          email: currentUser.email || 'user@example.com',
          tier: currentUser.isAnonymous ? 'guest' : 'free',
          remainingPdfExports: 5,
          remainingPngExports: 0,
          remainingAtsChecks: 0,
        });

        // Automatically go to dashboard on login if we were on home or login-related pages
        if (currentPage === 'home') {
          setCurrentPage('dashboard');
        }
      } else {
        // Only clear if we were using a real firebase user
        // Avoid clearing demo user
        setUser((prev: any) => {
  ...
  const handleDemoLogin = async () => {
    ...
    // Load local list for demo user too
    const list = await refreshResumesList(demoUser.uid, true);
    if (list.length > 0) {
      setActiveResumeId(list[0].id);
      setData(list[0].data);
      lastSavedDataRef.current = JSON.stringify(list[0].data);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        await saveResumeWithId(user.uid, activeResumeId, name, data, score, status, !!(user.isDemo || user.isAnonymous), templateId);
  ...
  const handleRenameResume = async (id: string, newName: string) => {
    if (!user) return;
    try {
      await renameResume(user.uid, id, newName, !!(user.isDemo || user.isAnonymous));
      await refreshResumesList(user.uid, !!(user.isDemo || user.isAnonymous));
    } catch (err) {
  ...
  const handleDeleteResume = async (id: string) => {
    if (!user) return;
    try {
      await deleteResume(user.uid, id, !!(user.isDemo || user.isAnonymous));
      const list = await refreshResumesList(user.uid, !!(user.isDemo || user.isAnonymous));
  ...
  const renderPage = () => {
    switch (currentPage) {
            ...
            onSelect={async (id) => { 
              try {
                ...
                if (!currentUser) {
                  ...
                  await refreshResumesList(demoUser.uid, true);
                }
                ...
                let newId;
                try {
                  newId = await createNewResume(currentUser.uid, newName, !!(currentUser.isDemo || currentUser.isAnonymous), id, initialIndustryData);
                } catch (err) {
                  console.error("Failed to create resume in storage", err);
                  newId = 'temp_' + Math.random().toString(36).substring(2, 9) + '_' + currentUser.uid;
                }
                ...
                try {
                  await refreshResumesList(currentUser.uid, !!(currentUser.isDemo || currentUser.isAnonymous));
                } catch (err) {
      ...
      case 'pricing':
        return <PricingPage currentTier={appUser?.tier || 'guest'} onSelectPlan={(id) => { setSelectedPlanId(id); setCurrentPage('payment'); }} />;
      case 'payment':
        return (
          <PaymentPage 
      ...
      case 'dashboard':
        return (
          <DashboardPage 
            ...
            onGoToPayment={(planId) => { setSelectedPlanId(planId); setCurrentPage('payment'); }}
            ...
            <div className="relative group">
              ...
              <div className="relative glass rounded-[2rem] overflow-hidden shadow-2xl h-[calc(100vh-180px)] lg:h-[820px] p-2">
                <div className="h-full rounded-2xl overflow-hidden border border-slate-100">
                   <ResumePreview ref={componentRef} data={data} templateId={templateId} userTier={appUser?.tier || 'guest'} />
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-center gap-3 ml-2">
                  ...
                  >
                    ...
                    <span className="text-xs font-bold">
                      {user.isAnonymous ? '游客' : user.phoneNumber || user.email || '普通账户'}
                    </span>
                  </button>
                  <button
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-slate-600 font-medium">
              ...
              <li><button onClick={() => setCurrentPage('builder')} className="hover:text-blue-600 transition-colors">在线制作</button></li>
